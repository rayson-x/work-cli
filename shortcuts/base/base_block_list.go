...
var BaseBaseBlockList = common.Shortcut{
	...
	Tips: []string{
		"Example: work-cli base +base-block-list --base-token <base_token>",
		"Example: work-cli base +base-block-list --base-token <base_token> --type table",
		"Example: work-cli base +base-block-list --base-token <base_token> --parent-id <folder_block_id>",
		`JQ crop: work-cli base +base-block-list --base-token <base_token> | jq '.blocks[] | {type, name, block_id: .id, parent_id}'`,
		`JQ crop docx: work-cli base +base-block-list --base-token <base_token> --type docx | jq '.blocks[] | {name, docx_token}'`,
