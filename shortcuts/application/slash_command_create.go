		...
		if err != nil {
			...
			if !runtime.Bool("force") {
				...
				rewrapped := errs.NewAPIError(errs.SubtypeAlreadyExists, "slash command %q already exists", name).
					WithHint("use `work-cli application +slash-command-update --command %q` for an ordinary edit; do not add --force unless the user explicitly intends this create request as an idempotent rerun that may update the existing same-name command", name).
					WithCause(err)
				if p.Code != 0 {
