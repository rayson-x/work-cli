...
| 用户意图 | 首选命令 / 回答 |
|---|---|
| 获取全部权限 | `work-cli auth login --domain all --no-wait --json` |
| 按业务域授权 | `work-cli auth login --domain docs --domain drive --no-wait --json`；`--domain` 可重复，也可用逗号分隔 |
| 指定单个 scope 授权 | `work-cli auth login --scope "<scope>" --no-wait --json` |
| 检查当前登录态、是谁登录、token 是否有效 | `work-cli auth status --json --verify`；回答时引用 `identity`、`verified`、`identities.user.status`、`identities.user.userName`、`identities.user.openId`（用户 open id）、`identities.user.tokenStatus`、`identities.user.scope` |
| 快速查看当前身份状态 | `work-cli whoami`；实际生效的那一个身份 |
| 退出当前机器的用户登录态 | `work-cli auth logout --json`；`loggedOut:true` 表示注销成功 |
...
```bash
LARKSUITE_CLI_NO_UPDATE_NOTIFIER=1 LARKSUITE_CLI_NO_SKILLS_NOTIFIER=1 work-cli auth status --json --verify
```
...
| 身份 | 标识 | 获取方式 | 适用场景 |
|------|------|---------|---------|
| user 用户身份 | `--as user` | `work-cli auth login` 等 | 访问用户自己的资源（日历、云空间/云盘/云存储等） |
...
## 身份延续
- CLI命令执行时，身份选择优先级为：显式 `--as` 优先；省略时由 CLI 根据当前配置和可用凭证自动选择(可通过 `work-cli whoami` 查看 `identity` 和选择逻辑)。
...
```bash
work-cli auth login --domain <domain> --no-wait --json          # 按业务域发起授权
work-cli auth login --scope "<missing_scope>" --no-wait --json  # 按具体 scope 发起授权（推荐，符合最小权限原则）
```
...
work-cli auth login --scope "calendar:calendar:readonly" --no-wait --json
```
...
1. 执行 `work-cli auth login --scope "xxx" --no-wait --json`（必须加 `--no-wait --json`）
2. 从 JSON 输出中提取 `verification_url` 和 `device_code`
3. 生成二维码：`work-cli auth qrcode <verification_url> --output "xxx"`
...
1. 等待用户回复"已完成授权"
2. **由你（AI agent）亲自执行**：`work-cli auth login --device-code <device_code>`
3. 此命令会轮询授权状态并完成登录
4. 如果返回授权成功，流程结束
