...
When creating or updating a formula field with `work-cli base +field-create/+field-update --json ...` and `type` is `formula`, you should read this guide first and only then add `--i-have-read-guide` to the command.
...
1. Get all table names: `work-cli base +table-list --base-token <base>` — returns `items[].table_name`
2. Get table structure: `work-cli base +table-get --base-token <base> --table-id <table>` — returns `fields[]`
