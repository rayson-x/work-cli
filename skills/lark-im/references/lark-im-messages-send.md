...
This skill maps to the shortcut: `work-cli im +messages-send` (internally calls `POST /open-apis/im/v1/messages`).
...
work-cli im +messages-send --chat-id oc_xxx --markdown $'## Report\n\n![diagram](img_v3_xxxx)\n\nSee above for details.'
```
...
```bash
work-cli im +messages-send --chat-id oc_xxx --text $'Build failed\nBranch: feature/im-docs\nAction: please check logs'
```

```bash
work-cli im +messages-send --chat-id oc_xxx --text $'```bash\nmake test\nmake lint\n```'
```
...
work-cli im +messages-send --chat-id oc_xxx --markdown $'## Update\n\n- item 1\n- item 2'

# Send a plain one-line message
work-cli im +messages-send --chat-id oc_xxx --text "Hello"

# Equivalent manual JSON
work-cli im +messages-send --chat-id oc_xxx --content '{"text":"Hello"}'

# Send to a direct message (pass open_id)
work-cli im +messages-send --user-id ou_xxx --text "Hello"

# Send multi-line text while preserving formatting
work-cli im +messages-send --chat-id oc_xxx --text $'Line 1\nLine 2\n  indented line'
...
work-cli im +messages-send --chat-id oc_xxx --markdown $'## Status\n\n![screenshot](img_v3_xxxx)\n\nDone.'

# If you need exact post structure, send JSON directly
work-cli im +messages-send --chat-id oc_xxx --msg-type post --content '{"zh_cn":{"title":"Title","content":[[{"tag":"text","text":"Body"}]]}}'

# Send a local image (uploaded automatically before sending)
work-cli im +messages-send --chat-id oc_xxx --image ./photo.png

# Or send directly with an existing image_key
work-cli im +messages-send --chat-id oc_xxx --image img_xxx

# Send a local file (uploaded automatically before sending)
work-cli im +messages-send --chat-id oc_xxx --file ./report.pdf

# Send a video (--video-cover is required as the cover)
work-cli im +messages-send --chat-id oc_xxx --video ./demo.mp4 --video-cover ./cover.png
work-cli im +messages-send --chat-id oc_xxx --video ./demo.mp4 --video-cover img_xxx

# Send a voice message
work-cli im +messages-send --chat-id oc_xxx --audio ./voice.opus

# Use an idempotency key (same key sends only once within 1 hour)
work-cli im +messages-send --chat-id oc_xxx --text "Hello" --idempotency-key my-unique-id

# Preview the request without executing it
work-cli im +messages-send --chat-id oc_xxx --markdown $'## Test\n\nhello' --dry-run
...
work-cli im +messages-send --chat-id oc_xxx --msg-type interactive --content '<card_json_from_workflow>'
```
