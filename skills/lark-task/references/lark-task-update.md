...
```bash
# Update task summary
work-cli task +update --task-id "<task_guid>" --summary "New Summary"

# Update multiple tasks' due dates
work-cli task +update --task-id "<task_guid>,<another_task_guid>" --due "+2d"

# A task applink is accepted directly; the CLI extracts its guid query value
work-cli task +update --task-id "https://applink.larksuite.com/client/todo/task?guid=<task_guid>" --summary "New Summary"

# Update with JSON data
work-cli task +update --task-id "<task_guid>" --data '{"description": "New description"}'
```
...
2. Execute `work-cli task +update --task-id "..." ...`
