...
**身份：Markdown 文件通常属于用户云空间资源，优先使用 `--as user`。首次以 `user` 身份访问前执行 `work-cli auth login`。**
...
- 用户要先拿 Markdown 文件的历史版本号，再做比较、下载或回滚，先用 [`lark-drive`](../../skills/lark-drive/SKILL.md) 的 `work-cli drive +version-history`
- 用户要把本地 Markdown **导入成在线新版文档（docx）**，不要用本域，改用 [`lark-drive`](../../skills/lark-drive/SKILL.md) 的 `work-cli drive +import --type docx`
